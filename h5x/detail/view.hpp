...
template<typename T>
auto h5_data_type(H5::DataSet const& dataset) -> typename std::enable_if<!boost::fusion::traits::is_sequence<T>::value, H5::DataType>::type
{
...
class View<T, 1>
{
   struct DataSet
   {
      DataSet(H5::DataSet const& dataset) : ds{dataset}, type{h5_data_type<T>(dataset)}, space{dataset.getSpace()}
      {
         npoints = space.getSimpleExtentNpoints();
         assert(type.getSize() == sizeof(T));
      }
      H5::DataSet const& ds;
      H5::DataType type;
